...
import { readLocalConfigFile } from './imgConfig'
...
function searchImgs(basePath: string, includeFolders: string[], excludeFolders: string[], webview: Webview) {
  ...
  console.info('[imgUtils]img load start!')
  searchFolders.forEach((folder) => {
    dfs(folder, (filePath: string) => {
      ...
      const vscodePath = webview.asWebviewUri(Uri.file(filePath)) + ''
  ...
  console.info('[imgUtils]img load finished!')
  return imgs
}
...
export const getImageSize = (filePath: string): { width: number, height: number } => {
  ...
  try {
    dimensions = imageSize(filePath) as any
  } catch (err) {
    console.log(err)
  }
  return dimensions
